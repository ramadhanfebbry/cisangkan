...
//----------------Penomoran Kode Buku----------------
function format_buku_1($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('buku_kelas_1');    
    $leadingzeros = '000';
    $no_buku = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_buku;
}

function format_buku_2($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('buku_kelas_2');    
    $leadingzeros = '000';
    $no_buku = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_buku;
}

function format_buku_3($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('buku_kelas_3');    
    $leadingzeros = '000';
    $no_buku = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_buku;
}

function format_buku_4($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('buku_kelas_4');    
    $leadingzeros = '000';
    $no_buku = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_buku;
}

function format_buku_5($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('buku_kelas_5');    
    $leadingzeros = '000';
    $no_buku = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_buku;
}

//----------------Akhir Penomoran Kode Buku----------------

//----------------Penomoran Kode Pengguna (Anggota Perpus & Pustakawan)----------------
function format_no_anggota($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('anggota');    
    $leadingzeros = '000';
    $no_pengguna = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_pengguna;
}

function format_pustakawan_1($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('pustakawan_1');    
    $leadingzeros = '000';
    $no_pengguna = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_pengguna;
}

function format_pustakawan_2($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('pustakawan_2');    
    $leadingzeros = '000';
    $no_pengguna = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_pengguna;
}

function format_pustakawan_3($no)
{
    // Ambil nilai prefix dari file konfigurasi "perpus.php"
    $prefix = config_item('pustakawan_3');    
    $leadingzeros = '000';
    $no_pengguna = $prefix . substr($leadingzeros, 0, (-strlen($no))) . $no;
    return $no_pengguna;
}
...
function get_nomor_buku($no_buku)
{
    return intval(substr($no_buku, 3, 3));
}

function get_nomor_pengguna($no_pengguna)
{
    return intval(substr($no_pengguna, 3, 3));
}
