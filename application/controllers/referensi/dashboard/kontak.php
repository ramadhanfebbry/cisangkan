<?php
class Kontak extends Dashboard_Controller
{
    public $data = array(
        'halaman' => 'kontak',
    ...
    public function index()
    {
        ...
        if (! $this->kontak->validate('form_rules')) {
            ...
            $this->data['form_action'] = site_url('dashboard/kontak');
        ...
        if ($this->kontak->simpan($kontak)) {
            $this->session->set_flashdata('pesan', 'Pesan berhasil dikirim. Balasan akan dikirim ke email Anda.'.anchor(site_url('dashboard'), ' Kembali ke halaman home.', 'class="alert-link"'));
            redirect('dashboard/kontak-sukses');
        } else {
            $this->session->set_flashdata('pesan_error', 'Maaf, pengiriman pesan gagal. Kami mengalami gangguan, coba ' . anchor('dashboard/kontak', 'ulangi', 'class="alert-link"') .' beberapa saat lagi.');
            redirect('dashboard/kontak-error');
        }
    }
