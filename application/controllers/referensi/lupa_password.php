...
class Lupa_Password extends Public_Controller
{
    ...
    public function index()
    {
        ...
        $peserta = (object) $this->input->post(null, true);
        $where = array(
            'id' => get_no_peserta_value($peserta->no_peserta),
            'nisn'  => $peserta->nisn,
            'email' => $peserta->email,
        );
        ...
        if (! $this->lupa_password->reset($where)) {
            $this->session->set_flashdata('pesan_error', 'Proses gagal, segera'. anchor('kontak', 'hubungi Operator / Panitia PSB.', 'class="alert-link"'));
            redirect('lupa-password-error');
        } else {
